              ...
              {logs.map((log) => (
                <div key={log.id} className="log-card">
                  ...
                  <div className="log-details">
                    <p><strong>Datum:</strong> {formatDate(log.created_at)}</p>
                    {log.workers && (
                      <p><strong>Mitarbeiter:</strong> {log.workers.first_name} {log.workers.last_name}</p>
                    )}
                    {log.notes && (
                      <p><strong>Notizen:</strong> {log.notes}</p>
                    )}
                  </div>

                  {log.status === 'completed' && log.signature && (
                    <div className="log-signature">
                      <p><strong>Unterschrift vorhanden</strong> ✓</p>
                    </div>
                  )}
                </div>
              ))}
