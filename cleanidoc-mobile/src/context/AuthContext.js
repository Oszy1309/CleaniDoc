...
export const AuthProvider = ({ children }) => {
  ...
  const signOut = async () => {
    try {
      console.log('🚪 AuthContext: Abmeldung mit Supabase...');
      const { error } = await supabase.auth.signOut();

      if (error) {
        console.error('❌ AuthContext: Supabase signOut Fehler:', error);
      } else {
        console.log('✅ AuthContext: Supabase signOut erfolgreich');
      }

      // State immer zurücksetzen, auch bei Fehlern
      setUser(null);
      setWorker(null);
      console.log('🔄 AuthContext: State zurückgesetzt');

      return { error };
    } catch (err) {
      console.error('❌ AuthContext: Unerwarteter signOut Fehler:', err);
