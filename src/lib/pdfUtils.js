...
export const generateCleaningLogPDF = (logData, filename) => {
  ...
  if (logData.planDescription) {
    ...
    const splitDescription = pdf.splitTextToSize(logData.planDescription, contentWidth - 30);
  ...
  logData.steps.forEach((step, index) => {
    ...
    const stepName =
      step.step_name.length > 25 ? step.step_name.substring(0, 22) + '...' : step.step_name;
    ...
    if (step.worker_notes && step.worker_notes.trim()) {
      const notes =
        step.worker_notes.length > 40
          ? step.worker_notes.substring(0, 37) + '...'
          : step.worker_notes;
...
export const generatePlanPDF = (planData, filename) => {
  ...
  if (planData.admin_signed_at) {
    pdf.text(
      `Unterzeichnet: ${new Date(planData.admin_signed_at).toLocaleDateString('de-DE')}`,
      margin,
      yPosition
    );
  ...
  if (planData.customer_signed_at) {
    pdf.text(
      `Unterzeichnet: ${new Date(planData.customer_signed_at).toLocaleDateString('de-DE')}`,
      margin,
      yPosition
    );
