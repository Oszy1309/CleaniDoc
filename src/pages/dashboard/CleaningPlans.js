...
import CleaningPlanModal from '../../components/forms/CleaningPlanModal';
import CleaningPlanDetail from '../../components/features/CleaningPlanDetail';
...
function CleaningPlans() {
  ...
  const handleDuplicatePlan = async plan => {
    try {
      ...
      const { error } = await supabase.from('cleaning_plans').insert([newPlan]);
  ...
  const handleDeletePlan = async planId => {
    if (
      !window.confirm(
        'Reinigungsplan wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.'
      )
    ) {
      return;
    }
      ...
      <div className="page-header">
        <div className="header-content">
          ...
          <p className="subtitle">
            Verwalten Sie Ihre standardisierten Reinigungsabläufe und Checklisten
          </p>
        </div>
            ...
            >
              <div className="plan-card-header">
                <div className="plan-title">
                  <h3>{plan.name}</h3>
                  {plan.category && <span className="category-badge">{plan.category}</span>}
                </div>
