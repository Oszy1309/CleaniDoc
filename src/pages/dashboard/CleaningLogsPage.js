  ...
  const generateLogsForDate = async date => {
    try {
      ...
      allPlans.forEach(plan => {
        console.log(
          `📋 Plan: "${plan.name}" | Status: "${plan.status}" | Frequency: "${plan.frequency}"`
        );
      });

      // Filtere nach status 'active' ODER 'fully_signed'
      const plans = allPlans.filter(
        plan => plan.status === 'active' || plan.status === 'fully_signed'
      );
      ...
      const relevantPlans = plans.filter(plan => {
        ...
        if (plan.day_of_week) {
          ...
          console.log(
            `📅 Plan Tage: ${days.join(', ')} → ${isRelevant ? 'relevant' : 'nicht relevant'}`
          );
          return isRelevant;
        }
      ...
      console.log(
        '📋 Debug: Bereits vorhandene Plan-IDs für',
        date,
        ':',
        Array.from(existingPlanIds)
      );
      ...
      if (existingLogs?.length > 0) {
        alert(
          `${logsToCreate.length} neue Logs erstellt für ${date}\n(${existingLogs.length} Logs existierten bereits)`
        );
      } else {
  ...
  const handleDeleteLog = async logId => {
    if (window.confirm('Log wirklich löschen?')) {
      try {
        ...
        await supabase.from('cleaning_log_steps').delete().eq('cleaning_log_id', logId);

        // Dann den Log
        const { error } = await supabase.from('cleaning_logs').delete().eq('id', logId);
    ...
    <div className="cleaning-logs-page">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <button className="btn-secondary" onClick={() => generateLogsForDate(selectedDate)}>
            Logs generieren
          </button>
        </div>
      ) : (
