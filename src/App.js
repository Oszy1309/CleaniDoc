...
function App() {
  ...
  // Check if token exists in localStorage (for handling navigation race condition)
  ...
  // Render admin viewing as employee
  if (viewAsRole === 'employee') {
    return (
      <Routes>
        <Route
          path="/"
  ...
  // Render admin viewing as customer
  if (viewAsRole === 'customer') {
    return (
      <Routes>
        <Route
          path="/"
          element={
            <CustomerDashboard user={user} onLogout={() => setViewAsRole(null)} isPreview={true} />
          }
        />
        <Route
  ...
  // Render employee/worker dashboard
  if (role === 'employee') {
    return (
      <Routes>
