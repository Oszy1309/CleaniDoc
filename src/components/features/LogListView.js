  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`${selectedLogs.size} Logs wirklich löschen?`)) {
      try {
        ...
        // Dann die Logs
        const { error } = await supabase.from('cleaning_logs').delete().in('id', logIds);
        ...
        {logs.map(log => (
          ...
          >
            <div className="log-header">
              ...
              <div className="log-status-badge" style={getStatusStyle(log.status)}>
            ...
            <div className="log-info">
              <p>
                <strong>Plan:</strong> {log.cleaning_plans?.name}
              </p>
              <p>
                <strong>Datum:</strong> {log.scheduled_date}
              </p>
              {log.created_at && (
                <p>
                  <strong>Erstellt:</strong> {new Date(log.created_at).toLocaleString('de-DE')}
                </p>
              )}
              {log.completed_at && (
                <p>
            ...
            <div className="log-actions">
              <button className="btn-icon edit" onClick={() => onSelectLog(log)} title="Bearbeiten">
                <Edit2 size={18} />
              </button>
              <button
