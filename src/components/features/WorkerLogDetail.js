      ...
      <div className="log-detail-content">
            ...
            {logSteps.map((step, index) => (
              <div key={step.id} className="step-item">
                ...
                <div className="step-details">
                  ...
                  {step.cleaning_agent && <p className="agent">Mittel: {step.cleaning_agent}</p>}
                  {step.dwell_time_minutes > 0 && (
                    <p className="dwell">Einwirkzeit: {step.dwell_time_minutes} min</p>
                  )}
                  {step.completed_at && (
                    <p className="completion-time">
                      Abgeschlossen: {new Date(step.completed_at).toLocaleTimeString('de-DE')}
                    </p>
                  )}
                </div>
        ...
        <div className="actions">
          ...
          {logSteps.every(s => s.completed) && !log.signature && (
            <button className="btn-success" onClick={() => setShowSignature(true)}>
              ✍️ Unterzeichnen
            </button>
          )}
        </div>
