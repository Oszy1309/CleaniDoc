...
import { Home, Users, FileText, ClipboardList, CheckSquare, LogOut, Menu, X } from 'lucide-react';
...
function ResponsiveDrawer({ onLogout, userRole = 'admin' }) {
  ...
  return (
    <>
      ...
      {isMobile && (
        <button className="drawer-toggle" onClick={toggleDrawer} aria-label="Toggle navigation">
          {isOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      )}

      {/* Backdrop for mobile */}
      {isMobile && isOpen && <div className="drawer-backdrop" onClick={() => setIsOpen(false)} />}

      {/* Drawer Content */}
      <aside
        className={`responsive-drawer ${isMobile ? 'drawer-mobile' : 'drawer-desktop'} ${isOpen ? 'drawer-open' : ''}`}
      >
          ...
          <div className="drawer-header">
            <div className="drawer-logo">
              <svg width="40" height="40" viewBox="0 0 80 80" className="logo-icon">
                ...
                <path
                  d="M30 25 L50 25 L50 35 L40 35 L40 45 L50 45 L50 55 L30 55 L30 45 L35 45 L35 35 L30 35 Z"
                  fill="white"
                />
              </svg>
